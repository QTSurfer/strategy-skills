...
Inner classes that implement `OnChangeRTIndicator.OnChangeListener` **directly** and call `emitSignal()` on the enclosing strategy will throw `IllegalAccessError` in production. This is a JVM access-control issue that only manifests in the fat-jar runtime (not in tests).

**Always extend `AbstractOnChangeListener`** instead of implementing the interface directly. This is the only safe pattern:

```java
// CORRECT — extends AbstractOnChangeListener
private class MyListener extends AbstractOnChangeListener {
    MyListener(AbstractTickerStrategy s, InstrumentGroupRTIndicator ind) { super(s, ind); }

    @Override
    public void onChange(StateStoreSupport store, double prev, double actual) {
        emitBuy(actual); // safe — inherited from AbstractOnChangeListener
    }
}

// WRONG — raw interface, causes IllegalAccessError in production
private class MyListener implements OnChangeRTIndicator.OnChangeListener {
    @Override
    public void onChange(StateStoreSupport store, double prev, double actual) {
        emitBuy(actual); // ← IllegalAccessError at runtime
    }
}
```
